...
import configureSideMenu from './scripts/sideMenu'
import configureHeaderTabs from './scripts/headerTabs'
import createCards from './scripts/createCards'
import setupMyShipPage from './scripts/setupMyShipPage'
...
const addListenersToSearchInputs = () => {

  const handleInputBlur = () => {
    alert('Blur not implemented yet)')
  }

  document.getElementById('max-price-input').addEventListener('blur', handleInputBlur)
  document.getElementById('min-price-input').addEventListener('blur', handleInputBlur)
  document.getElementById('max-speed-input').addEventListener('blur', handleInputBlur)
  document.getElementById('min-speed-input').addEventListener('blur', handleInputBlur)
}

const addCardsToDom = () => {
  const cardsHolder = document.createElement('div')
  cardsHolder.setAttribute('id', 'cards-holder')
  cardsHolder.classList.add('cards-holder')

  const cards = createCards(ships)
  cards.forEach(card => cardsHolder.append(card))

  const bightTab = document.getElementById('bight-tab')
  bightTab.append(cardsHolder)
}
